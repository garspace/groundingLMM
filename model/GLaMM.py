    ...
    def _generate_and_postprocess_masks(self, pred_embeddings, image_embeddings, resize_list, label_list, infer=False):
        ...
        for i, pred_embedding in enumerate(pred_embeddings):
            ...
            low_res_masks, _ = self.model.grounding_encoder.mask_decoder(
                ...
                # multimask_output=False, ) # output single mask for training
                multimask_output=False if not infer else False) # output multi mask for inference
            ...
            if not infer:
              pred_masks.append(pred_mask[:, 0]) # for training
            else:
              pred_masks.append(pred_mask[0, :]) # for inference
        return pred_masks
